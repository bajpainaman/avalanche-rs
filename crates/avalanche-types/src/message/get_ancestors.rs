...
impl Message {
    ...
    #[must_use]
    pub fn container_id(mut self, container_id: ids::Id) -> Self {
        self.msg.container_id = prost::bytes::Bytes::from(container_id.to_vec());
        self
    }
    ...
    pub fn serialize(&self) -> io::Result<Vec<u8>> {
        let msg = p2p::Message {
            message: Some(p2p::message::Message::GetAncestors(self.msg.clone())),
        };
        ...
        if uncompressed_len > compressed_len {
            log::debug!(
                "get_ancestors compression saved {} bytes",
                uncompressed_len - compressed_len
            );
        } else {
            log::debug!(
                "get_ancestors compression added {} byte(s)",
    ...
    pub fn deserialize(d: impl AsRef<[u8]>) -> io::Result<Self> {
        ...
        match p2p_msg.message.unwrap() {
            // was not compressed
            p2p::message::Message::GetAncestors(msg) => Ok(Message {
                msg,
                gzip_compress: false,
            }),
...
/// RUST_LOG=debug cargo test --package avalanche-types --lib -- message::get_ancestors::test_message --exact --show-output
#[test]
fn test_message() {
