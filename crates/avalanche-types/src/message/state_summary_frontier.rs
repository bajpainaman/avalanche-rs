...
pub struct Message {
    pub msg: p2p::StateSummaryFrontier,
...
impl Default for Message {
    fn default() -> Self {
        Message {
            msg: p2p::StateSummaryFrontier {
                chain_id: prost::bytes::Bytes::new(),
                request_id: 0,
                summary: prost::bytes::Bytes::new(),
            },
            gzip_compress: false,
        }
    }
...
impl Message {
    ...
    #[must_use]
    pub fn summary(mut self, summary: Vec<u8>) -> Self {
        self.msg.summary = prost::bytes::Bytes::from(summary);
        self
    }
    ...
    pub fn serialize(&self) -> io::Result<Vec<u8>> {
        let msg = p2p::Message {
            message: Some(p2p::message::Message::StateSummaryFrontier(
                self.msg.clone(),
            )),
        };
    ...
    pub fn deserialize(d: impl AsRef<[u8]>) -> io::Result<Self> {
        ...
        match p2p_msg.message.unwrap() {
            // was not compressed
            p2p::message::Message::StateSummaryFrontier(msg) => Ok(Message {
                msg,
                gzip_compress: false,
            }),
