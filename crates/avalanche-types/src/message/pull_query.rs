...
impl Message {
    ...
    pub fn serialize(&self) -> io::Result<Vec<u8>> {
        let msg = p2p::Message {
            message: Some(p2p::message::Message::PullQuery(self.msg.clone())),
        };
        ...
        if uncompressed_len > compressed_len {
            log::debug!(
                "pull_query compression saved {} bytes",
                uncompressed_len - compressed_len
            );
        } else {
            log::debug!(
                "pull_query compression added {} byte(s)",
    ...
    pub fn deserialize(d: impl AsRef<[u8]>) -> io::Result<Self> {
        ...
        match p2p_msg.message.unwrap() {
            // was not compressed
            p2p::message::Message::PullQuery(msg) => Ok(Message {
                msg,
                gzip_compress: false,
            }),
...
/// RUST_LOG=debug cargo test --package avalanche-types --lib -- message::pull_query::test_message --exact --show-output
#[test]
fn test_message() {
