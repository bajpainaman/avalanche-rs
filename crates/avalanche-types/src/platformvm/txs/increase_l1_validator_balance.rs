...
impl Tx {
    ...
    pub async fn sign<T: key::secp256k1::SignOnly>(&mut self, signers: Vec<Vec<T>>) -> Result<()> {
        ...
        // Pack IncreaseL1ValidatorBalanceTx-specific fields
...
mod tests {
    ...
    /// Tests signing an IncreaseL1ValidatorBalanceTx.
    #[test]
    fn test_increase_l1_validator_balance_tx_sign() {
        macro_rules! ab {
            ($e:expr) => {
        ...
        let validation_id = Id::from_slice(&[14u8; 32]);
        ...
        let metadata = tx.base_tx.metadata.as_ref().unwrap();
        ...
        assert_eq!(tx.creds[0].signatures[0].len(), 65); // secp256k1 signature length

        // Verify unsigned and signed bytes are different (signed includes credentials)
        assert_ne!(
            metadata.tx_bytes_with_no_signature,
            metadata.tx_bytes_with_signatures
        );

        // Verify signed bytes is longer than unsigned (credentials were added)
        assert!(metadata.tx_bytes_with_signatures.len() > metadata.tx_bytes_with_no_signature.len());
    }
}
